...
if(_depthChange > sysUpdateRange) {
	...
	if(_depthChange < sysSpacing * sysCount) {
		...
		if(_stepSign == 1) {   
			_updateCamEdgeY += sysSpacing * sysCount - sysUpdateRange * 2; // sys range * 2?
			if(irandom(3) == 0) {
				//msg("running step sign adjustment");
			}
		} else {
			if(irandom(3) == 0) {
				//msg("NOT RUNNING STEP ADJUST");
			}
		}
		...
		if(_stepSign == -1) {
			part_system_depth(_sysCollection[_updatePos], _updateDepth + sysSpacing);
		...
		setCollectionPosition(); // TODO needs to be made work with new position
	}
