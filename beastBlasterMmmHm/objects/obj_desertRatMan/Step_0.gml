...
if(alive == 1 && !ragdolling) {
	if(instance_exists(agroId) && agroId.alive) {
		if(irandom(32) == 0) { // only update the intent and behavior 4 times a second			
			...
			var _dist = point_distance(x, y, agroId.x, agroId.y);
			var _dir = point_direction(x, y, agroId.x, agroId.y);
			
			var _approachSpeed = clamp(sqrt(_dist / 370) - 1, -1.7, 1.4);
			...
			if(_dist < 750) {
				if(irandom(2) == 0) {
					script_shootBullet(x, y, 15 / 2, _dir, obj_basicBullet,, 15 / 2 * (100 / point_distance(x, y, agroId.x, agroId.y)),,.12);
				}
				if(irandom(120) == 0) {
		...
		if(friendly) {
			if(irandom(18) == 0) {
				...
				if(_player != noone) {
					followId = _player;
					var _followX = _player.x + _player.xChange * 16;
					var _followY = _player.y + _player.xChange * 16;
					var _dist = point_distance(x, y, _followX, _followY);
					var _dir = point_direction(x, y, _followX, _followY);
			
					var _approachSpeed = clamp(_dist / 8 - 1, -2.8, 2.8);
					...
					collision_circle_list(x, y, 50, obj_enemy, false, true, _avoidList, true);
