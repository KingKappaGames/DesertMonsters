...
function script_drawWeapon(sprite, position, angle, holdDownAngleAdjust){
	var _gunLayers = sprite_get_number(sprite);
	var _yAdd = -3; // the up push of each layer
	
	for(var _i = 0; _i < _gunLayers; _i++) {
		draw_sprite_ext(sprite, _i, position[0], position[1] + _yAdd * _i, .65 + abs(dcos(angle)) * .35, 1, angle + holdDownAngleAdjust, c_white, 1);
	}
}