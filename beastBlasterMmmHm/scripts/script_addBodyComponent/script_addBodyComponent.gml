...
///@param xscale The horizontal scale of the image (pre compressions)
///@param yscale The vertical scale of the image (pre compressions)
...
function script_addLimbBodyComponent(creature, spineIndex, spriteArr, imageArr, rotRelative, heightSet, distanceSet, xscaleArr, yscaleArr, viewAngleSet, viewCompressionMinimum, colorArr = c_white, imgFromRotationAdjust = 0, limbArrRef, gunHoldOffsetArr) constructor { // these limbs still need to adhere to rotation and position values because the sorting will use it the same way for all, or I can change it but seems easy enough as is
	sprite = spriteArr;
	image = imageArr; 
	rotationRelative = rotRelative; 
	height = heightSet; 
